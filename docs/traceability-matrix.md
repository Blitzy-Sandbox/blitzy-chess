...
| 6 | Transposition table uses the 64-bit Zobrist key (the constraint wording said `board.zobrist_hash()`), not the internal `_transposition_key()` | `backend/chess_ai/engine/search.py` | Keyed on the 64-bit integer from `chess.polyglot.zobrist_hash(board)` — python-chess 1.11.2 exposes no `Board.zobrist_hash()` method, so the polyglot helper supplies the key; the internal `_transposition_key()` (a tuple) is not used; table capped at 256 MB / 2^20 entries |
