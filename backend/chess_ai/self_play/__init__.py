...
Contains the orchestration runner (``runner``, runnable as
``python -m chess_ai.self_play.runner``), which drives the browser through
Playwright and offloads the engine search, and the commentary transcript
writer (``annotator``), which is standard-library only.
